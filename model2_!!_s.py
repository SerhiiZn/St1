...
#df=pd.read_sql_query('SELECT results.id, results.subject, results.score, results.expectation, results.title, results.examined_at, results.is_in_class, student_years.student_id, student_years.academical_year, student_years.calendar_year,  students.school_id, students.gender, students.year_of_entry FROM results INNER JOIN student_years ON student_years.id = results.student_year_id INNER JOIN students ON students.id = student_years.student_id WHERE results.is_in_class=False', cnx)
df=pd.read_sql_query('SELECT results.id, results.subject, results.Valuation, results.expectation, results.title, results.examined_at, results.is_in_class, student_years.student_id, student_years.academical_year, student_years.calendar_year,  students.school_id, students.gender, students.year_of_entry FROM results INNER JOIN student_years ON student_years.id = results.student_year_id INNER JOIN students ON students.id = student_years.student_id WHERE (results.Valuation IS NOT NULL) and (results.is_in_class=False)', cnx)
          ...
          #
              ...
              #
                          try:
                              ...
                              forecast_score= float(formula.A0.values[0][0])+float(formula.A1.values[0][0][0])*float(X_.valuation.values[-1]) + float(formula.A1.values[0][0][1])*float(X_.score_start.values[-1]) + float(formula.A1.values[0][0][2])*X_.academical_year.values[-1]
                              #print(forecast_score)
                          except:
                                  forecast_score= None
                    else:
                        #print('regr 2')
                        forecast_score= float(formula.A0.values[0][0])+float(formula.A1.values[0][0][0])*float(X_.valuation.values[-1]) + float(formula.A1.values[0][0][1])*X_.academical_year.values[-1]
...
#                 #   
          ...
          if start_exam==True and flag==True:
             ...
             b=X_[['valuation','score_start', 'academical_year']]
             #print(b)
             #print('!')
             #print(b[0:1])
#             forecast_score=loaded_model.predict(b.reshape(1, -1))
             ...
             b=[[X_.valuation.values[-1], X_.academical_year.values[-1]]]
             #print(b)
             #b=np.array([b])
             loaded_model = pickle.load(open(str(namefile[0]), 'rb'))
             #forecast_score=loaded_model.predict(b.reshape(1, -1))
...
def student_(student_data):
    ...
    for i in list_subjects:
        ...
        if len(df_student)>0:
            ...
            df_student['expectation_exp'] = df_student.apply(lambda row: dX_exp(df_student, row['valuation'], row['examined_at']), axis=1)
...
def create_models(df, start_exam):
   ...
   # 
        for j in range(len(spisok)):
            if start_exam==True:
               m=s[j][['student_id','examined_at', 'valuation', 'score_start', 'academical_year']]
               #print[m]
            else:
               #print('+') 
               m=s0[j][['student_id','examined_at', 'valuation', 'academical_year']]
               #print[m]
   #rename column 'score' on name of subject   
            first=m.rename(columns = {'valuation': spisok[j]}).sort_values('examined_at')
            #
   ...
   #merge DF for first data subject and second data subject
            for i in range(len(spisok)):
                if start_exam==True:
                    m=s[i][['student_id','examined_at', 'valuation', 'score_start', 'academical_year']]
                else:
                   # print('++') 
                    m=s0[i][['student_id','examined_at', 'valuation', 'academical_year']]
             
         #rename column 'score' on name of subject 
                second=m.rename(columns = {'valuation': spisok[i]}).sort_values('examined_at')
                #print(first[first.student_id==7181])
                #print('F')
...
#                if len(data_all[j][i])>6 and (abs(data_all[j][i].corr().iloc[2].filter(regex=data_all[j][i].columns[2])[0])>0.15) and (j!=i):
                if len(data_all[j][i])>20 and (j!=i):
...
df0=df0.sort_values(['subject', 'examined_at'])
#df0=df0[-df0.Valuation.isin(chars_)]
#df=df[-df.Valuation.isin(chars_)]
for i in spisok:
    #
